...
- The `.gitignore` file recognizes folder patterns and ignores files or folder paths based on what you provide. 
    ...
    - Botched first venv attempt left a `.gitignore` with an `*` inside it, which silently swallowed some files. Using `git status --ignored` and `git check-ignore -v <path>` assisted in narrowing it down.